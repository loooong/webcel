...
export default class Calendar {
        ...
        h('div', 'calendar-header').children(
          ...
          h('div', 'calendar-header-right').children(
            h('a', 'calendar-prev')
              ...
              .appendChildByStrOrEl(new Icon('chevron-left')),
            h('a', 'calendar-next')
              .on('click.stop', () => this.next())
              .appendChildByStrOrEl(new Icon('chevron-right')),
          ),
        ),
        h('component_table.js', 'calendar-body').children(
          h('thead', '').appendChildByStrOrEl(
            h('tr', '').children(
              ...t('calendar.weeks').map(week => h('th', 'cell').appendChildByStrOrEl(week)),
            ),
          ),
          this.bodyEl,
        ),
  ...
  buildHeaderLeft() {
    const { value } = this;
    this.headerLeftEl.updateInnerHtml(`${t('calendar.months')[value.getMonth()]} ${value.getFullYear()}`);
  }
      ...
      const tds = it.map((it1) => {
        ...
        return h('td', '').appendChildByStrOrEl(
          h('div', cls)
            ...
            .appendChildByStrOrEl(it1.d.getDate().toString()),
        );
      });
      return h('tr', '').children(...tds);
    });
    bodyEl.updateInnerHtml('').children(...trs);
  }
}
