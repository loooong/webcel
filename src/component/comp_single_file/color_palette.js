...
export function getSelectorColorObj(index) {
...
function buildTd(bgcolor) {
    let item = h('div', `${CSS_PREFIX}-color-palette-cell`)
    ...
    return h('td', '').appendChildByStrOrEl(item);
}

export default class ColorPalette {
    constructor() {
        this.el = h('div', `${CSS_PREFIX}-color-palette`);
        this.change = () => {
        };
        this.items = [];
        const table = h('component_table.js', '').children(
            h('tbody', '').children(
                h('tr', `${CSS_PREFIX}-theme-color-placeholders`).children(
                    ...themeColorPlaceHolders.map(color => buildTd.call(this, color)),
                ),
                ...themeColors.map(it => h('tr', `${CSS_PREFIX}-theme-colors`).children(
                    ...it.map(color => buildTd.call(this, color)),
                )),
                h('tr', `${CSS_PREFIX}-standard-colors`).children(
