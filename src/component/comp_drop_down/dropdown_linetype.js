...
export default class DropdownLineType extends Dropdown {
  constructor(type) {
    ...
    let beforei = 0;
    const lineTypeEls = lineTypes.map((it, iti) => h('div', `${CSS_PREFIX}-item state ${type === it[0] ? 'checked' : ''}`)
      .on('click', () => {
      ...
      .appendChildByStrOrEl(
        h('div', `${CSS_PREFIX}-line-type`).updateInnerHtml(it[1]),
      ));
