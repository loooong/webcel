...
import {DATE_PATTERN, str2Re} from "./config_reg_pattern";
import {formatNumberRender} from "../core/old_1/format";

import { isHave } from './check_value';
import { isNumber } from './dataproxy_helper';
...
export function dateDiff(date) {
    ...
    for (let i = 0; valid === false && i < DATE_PATTERN.length; i++) {
        valid = str2Re(DATE_PATTERN[i]).test(date);
    }
