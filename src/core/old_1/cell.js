...
import {look, look2} from "../../global_utils/config_for_core_and_component";
console.log(new Error("本文件将会删除！"))
// import XLSX_CALC from "solveExpression"
// var expression_fn = require('expression_fn');
// Converting infix expression to a suffix expression
// src: AVERAGE(SUM(A1,A2), B1) + 50 + B20
// return: [A1, A2], SUM[, B1],AVERAGE,50,+,B20,+
export const infixExprToSuffixExpr = (src) => { // 这个似乎是在做语法解析
    const operatorStack = [];
    const stack = [];
...
export function cellRender(data, sheetbook, y, x, src, formulaMap, getCellText, cellList = []){
    ...
    if (src[0] === '=' && src[1] !== '=') {
        if (evalFormula(src.substring(1), "wland") || evalFormula(src.substring(1), "wfr") || evalFormula(src.substring(1), "rtd")
            ...
            // let {date_formula} = data.coreSheetSetting;=MD.RTD("sina", "162411", "now")
