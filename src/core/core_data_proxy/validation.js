...
class Validation {
  ...
  includes(ri, ci) {
    ...
    for (let i = 0; i < refs.length; i += 1) {
      const cr = CellRangeProxy.fromRangeExpr(refs[i]);
  ...
  addRef(ref) {
    this.remove(CellRangeProxy.fromRangeExpr(ref));
    this.refs.push(ref);
  }
  ...
  remove(cellRange) {
    ...
    this.refs.forEach((it) => {
      const cr = CellRangeProxy.fromRangeExpr(it);
      if (cr.isOverlapWith(cellRange)) {
        const crs = cr.getDiffWithOther_Old(cellRange);
...
export class Validations {
  ...
  getByValidator(validator) {
    for (let i = 0; i < this._.length; i += 1) {
      const v = this._[i];
      if (v.validator.isEqualWithOther(validator)) {
        return v;
      }
    }
    return null;
