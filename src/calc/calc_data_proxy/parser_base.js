...
export class Str2NumberParser { // 把字符串转化为数字的类
  constructor(aStr) {
    this.originStr = aStr
    this.toParseStr = aStr; // 如果originStr是-23，toParseStr会变为23
  }
  ...
  preProcess(){
    let multiply = 1
    let theBeginChar = this.originStr.trim()[0]
    if(theBeginChar === "-"){
      multiply = -1
    }
    if(["-","+"].indexOf(theBeginChar) !== -1){
      this.toParseStr = this.originStr.slice(1).trim() // "  -  233"->"-233"
    }
    else {
      this.toParseStr = this.originStr.trim()
    }
    return multiply
  }

  easyParse2Number() {
    let multiply = this.preProcess()
