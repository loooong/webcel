...
import { UserRawFnExecutor } from './exp_fn_raw_fn_executor';
import { UserFnExecutor } from './exp_fn_executor';
import { BaseExpFunction } from './exp_fn_warp';
import { CellVError } from '../../internal_module/basic_cell_value';
import { ERROR_NAME } from '../calc_utils/error_config';
  ...
  getFnExecutorByName(fnName, isToUpperCase = true){
    ...
    if (this.isValidExpFn(foundExpFn)) {
      ...
      if (this.isValidExpFn(foundExpFn)) {
        ...
        // 到这一步是exp_fn没有找到；返回的函数是直接报 #NAME ERROR错误
        return  new this.rawFnExecutor((...arg) => new CellVError(new Error(ERROR_NAME))
        )
      }
    }
  }
